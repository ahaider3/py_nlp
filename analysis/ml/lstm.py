...
class LSTM:
  ...
  def forward(X, WLSTM, c0 = None, h0 = None):
    ...
    if c0 is None: c0 = np.zeros((b,d)) # batchsize X hidden size
    if h0 is None: h0 = np.zeros((b,d)) # batchsize X hidden size
    
    # Perform the LSTM forward pass with X as the input
    xphpb = WLSTM.shape[0] # x plus h plus bias, lol ; input+hidden+1
    ...
    C = np.zeros((n, b, d)) # cell content (seq, batch, hidden)
    Ct = np.zeros((n, b, d)) # tanh of cell content
    for t in xrange(n):
      # concat [x,h] as input to the LSTM
      prevh = Hout[t-1] if t > 0 else h0 # h0 return on first iter
      Hin[t,:,0] = 1 # bias
