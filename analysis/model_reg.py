...
#  cleaned_data = [(sent, tknzr.tokenize(tweet)) for sent, tweet in data]
 ...
 # x_plhr, y_plhr, loss, train_op = build_rnn(SEQ_LENGTH, BATCH_SIZE, STATE_SIZE, 300)
  x_plhr, y_plhr, loss, train_op = build_logreg(SEQ_LENGTH, BATCH_SIZE, STATE_SIZE, 300)

  sentiments = np.array([int(s[0].strip("\"")) for s in sent_vec]).reshape((len(sent_vec), 1)).astype(np.int32)
 ...
 #     fd[y_plhr] = sentiments[samples]
      one_hot[sentiments[samples][0]/2] = 1.0
